...
Example: After deprecating `meeting_prep_digest.py`, 7 planning docs still reference it.
...
5. Generate report: `N5/logs/maintenance/stale_refs_{date}.md`
...
```python
REFERENCE_PATTERNS = [
    r"file '([^']+)'",           # Zo file mentions: file 'path/to/file.py'
    r"`([^`]+\.py)`",             # Inline code: `script.py`
    r"script.*?([/\w]+\.py)",    # Script references
    r"file: ([^\s]+)",            # Plain file refs
]
```
...
These 7 files reference deprecated `meeting_prep_digest.py`:
1. `Documents/meeting-digest-BUILD-PLAN.md`
2. `Documents/HOWIE-ZO-IMPLEMENTATION-COMPLETE.md`
3. `N5/docs/HOWIE-ZO-HARMONIZATION-HANDOFF.md`
4. `N5/docs/PHASE-2-POLLING-PLAN.md`
5. `N5/commands/add-digest.md`
6. `N5/SESSION-SUMMARY-2025-10-09.md`
7. `Documents/weekly-summary-integration-analysis.md`
