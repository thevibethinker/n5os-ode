...
Usage:
    ...
    python3 orchestrator.py validate <project_path> [--fail-on-error]
"""
...
"""
    ...
    def validate_project(self, project_path: Path, fail_on_error: bool = True) -> Dict:
        """Run validation checks on project code."""
        try:
            ...
            logger.info(f"  Files scanned: {report.files_scanned}")
            logger.info(f"  Errors: {len(report.errors)}")
            logger.info(f"  Warnings: {len(report.warnings)}")
            
            if report.errors:
                logger.error("❌ Critical issues found:")
                for issue in report.errors[:5]:  # Show first 5
                    logger.error(f"  [{issue.category}] {issue.file_path.name}:{issue.line_number} - {issue.message}")
                if len(report.errors) > 5:
                    logger.error(f"  ... and {len(report.errors) - 5} more")
            
            if report.warnings:
                logger.warning("⚠️  Warnings found:")
                for issue in report.warnings[:3]:  # Show first 3
                    logger.warning(f"  [{issue.category}] {issue.file_path.name}:{issue.line_number} - {issue.message}")
                if len(report.warnings) > 3:
                    logger.warning(f"  ... and {len(report.warnings) - 3} more")
            
            # Write detailed report to orchestrator workspace
            report_file = self.workspace / "VALIDATION_REPORT.md"
            report_file.write_text(report.summary())
            logger.info(f"  Detailed report: {report_file}")
            
            result = {
                "status": "fail" if report.has_critical_issues else "pass",
...
def parse_args():
    ...
    parser.add_argument("action", choices=["assign", "check-worker", "review-changes", "approve", "test-integration", "validate"])
    parser.add_argument("task_or_worker", nargs="?", help="Task description (for assign) or worker ID or project path (for validate)")
    ...
    parser.add_argument("--fail-on-error", action="store_true", help="Exit with error code if validation finds issues")
    parser.add_argument(
        "--validate",
        action="store_true",
        help="Run validation checks after workers complete"
    )
    parser.add_argument(
        "--validation-config",
        type=Path,
...
def run_orchestrator(
    ...
    if args.action == "assign":
        ...
        if not args.task_or_worker:
            logger.error("Project path required for validate")
            return 1
        project_path = Path(args.task_or_worker)
        if not project_path.exists():
            logger.error(f"Project path not found: {project_path}")
            return 1
        result = orchestrator.validate_project(project_path, fail_on_error=args.fail_on_error)
        print(json.dumps(result, indent=2))
        if args.fail_on_error and result.get("status") == "fail":
            return 1
        return 0 if "error" not in result else 1
    ...
    if validate:
        ...
        result = validate_project(project_dir, validation_config)
        
        if result.has_errors:
            logger.warning("⚠ Validation found issues - review before proceeding")
            # Save validation results
            validation_output = project_dir / "VALIDATION_RESULTS.json"
            validation_output.write_text(json.dumps(result.to_dict(), indent=2))
