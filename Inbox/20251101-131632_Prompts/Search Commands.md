---
description: Search the N5 command registry (`Prompts/recipes.jsonl (index only)`) by keyword,
...
Search the N5 command registry (`Prompts/recipes.jsonl (index only)`) by keyword, category, or description. Helps discover available commands without manually reading the registry.
...
**Command Registry:** This command is registered in `Prompts/recipes.jsonl (index only)`  
...
- **Registry not found**: Returns error if `Prompts/recipes.jsonl (index only)` doesn't exist
...
- `file 'Prompts/recipes.jsonl (index only)'` - Command registry (source of truth)
