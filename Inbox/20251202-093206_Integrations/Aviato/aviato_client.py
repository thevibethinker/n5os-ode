...
class AviatoClient:
    ...
    def __init__(self, env_file: str = '/home/workspace/Integrations/Aviato/.env'):
        """Initialize Aviato client with credentials from .env file"""
        load_dotenv(env_file)
        # Prefer N5OS-specific secret first, then legacy name for compatibility
        env_key = os.environ.get("AVIATO_N5OS_V2_KEY") or os.environ.get("AVIATO_API_KEY")
        self.api_key = env_key
        if not self.api_key:
            raise RuntimeError("Aviato API key not found in environment (expected AVIATO_N5OS_V2_KEY or AVIATO_API_KEY)")
        self.base_url = os.getenv('AVIATO_API_BASE_URL', 'https://data.api.aviato.co').rstrip("/")
