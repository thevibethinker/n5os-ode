...
| `supports_position` | Stance | Edge evidence validates the owner's documented position (Phase 4.5) |
| `challenges_position` | Stance | Edge evidence contradicts the owner's documented position (Phase 4.5) |
...
This system uses a "Resonance Reservoir" to distinguish the owner's established mental models from genuinely novel ideas. The contextual primer (injected above when available) tells you what the owner already knows.
...
Before extracting any idea, ask: **"Is this genuinely new thinking, or the owner restating what he already believes?"**
...
- **Owner Identity: The system owner is always `owner` as person ID
