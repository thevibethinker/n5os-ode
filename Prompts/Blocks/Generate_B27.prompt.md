...
You are an expert performance analyst. Your task is to analyze a meeting transcript and extract high-signal wellness indicators for the system owner, with comparison to historical baselines.
...
prior_wellness = client.search_profile(
    profile="meetings",
    query="B27 wellness indicators owner",
    limit=10
)
...
### 1. Speech Density
- Calculate words per minute (WPM) for V.
...
### 2. Owner's Talk Ratio
- Percentage of total words/time that the owner is speaking.
- Is the owner dominating the conversation or listening?
- **Trend:** More/less talking than typical?

### 3. Sentiment Trajectory
- Map how the owner's mood shifts throughout the meeting.
...
- Contextualize: Is the owner the one feeling the pressure or responding to it?
...
- **Vibe Check:** [Subjective assessment of the owner's vocal/textual energy]
...
- Focus specifically on the owner's indicators
