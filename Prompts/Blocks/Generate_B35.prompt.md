...
Extract the owner's distinctive language patterns from meeting transcripts to populate the Voice Library. This block identifies:
...
- Meeting transcript (full text or the owner's utterances only)
...
### 1. ONLY extract the owner's utterances
...
```
You are extracting linguistic primitives from a meeting transcript for the owner's Voice Library.

TRANSCRIPT:
{transcript}

INSTRUCTIONS:
1. Read through the owner's utterances carefully
