...
- **The orchestrator is a document, not a conversation.** `BUILD.md` is the living record V returns to throughout the build.
- **Workers are separate conversations.** V manually opens new threads and pastes worker briefs.
- **State flows through files, not APIs.** Workers write completions to disk; the orchestrator reads them on V's return.
- **Human-in-the-loop at every step.** V decides when to launch workers, reviews completions, and approves adaptations.

This is distributed cognition with V as the coordinator, not fire-and-forget automation.
...
|------|---------|------------|-----------|
| `BUILD.md` | Living orchestrator doc; V returns here | Architect initially, updated throughout | V, Orchestrator persona |
| `PLAN.md` | Execution plan with waves, dependencies | Architect | V, all workers (context) |
| `STATUS.md` | Progress dashboard | Updated after each completion | V (quick status check) |
| `meta.json` | Machine state for scripts | Scripts, orchestrator | Scripts, close hooks |
| `workers/*.md` | Self-contained worker briefs | Architect | V pastes into new threads |
| `completions/*.json` | Structured completion reports | Worker close hooks | Orchestrator on V's return |
...
1. **V invokes build orchestrator** — Says "build X" or describes a complex system to create
...
5. **Architect initializes BUILD.md** — The living document V will return to

### Phase 2: Execution Loop

6. **V opens new thread** — Creates a fresh conversation
7. **V pastes worker brief** — The brief contains all context needed
...
11. **V returns to orchestrator** — Opens original conversation or fresh thread with BUILD.md
...
Before launching a worker, V (or the orchestrator) verifies:
...
- Answer questions if V has them
