...
### Setup
- [ ] Source data present in `source/`
- [ ] Identified source type (zip, DuckDB, SQLite, Parquet, JSON, CSV)

### Phase 1: Exploration
- [ ] Archives extracted to `source/extracted/` (if applicable)
- [ ] Sampled files to understand structure
- [ ] Identified key entities/tables to create
- [ ] Decided what to include vs. skip (scope)

### Phase 2: Ingestion
- [ ] `ingest/ingest.py` written
- [ ] Script runs without errors
- [ ] Fresh `data.duckdb` created at dataset root (not copied/renamed from source)
- [ ] All planned tables exist (`SHOW TABLES`)
- [ ] Row counts are reasonable (not 0, not wildly off)
- [ ] Sample queries return sensible data
- [ ] Timestamps parsed correctly (check MIN/MAX)
- [ ] No obvious data quality issues (nulls in key columns, etc.)

### Phase 3: Pipeline Documentation
- [ ] `ingest/INGEST.prompt.md` documents source files
- [ ] `ingest/INGEST.prompt.md` documents tables created
- [ ] `ingest/INGEST.prompt.md` documents any transformations

### Phase 4: README
- [ ] `README.md` includes "How to Get Your Data" export instructions
- [ ] `README.md` explains what this dataset is
- [ ] `README.md` describes coverage (time period, source)
- [ ] `README.md` explains business rules and semantics
- [ ] `README.md` includes 3-5 example queries with explanations

### Phase 5: Schema
- [ ] All tables have COMMENT ON TABLE descriptions in `ingest.py`
- [ ] All columns have COMMENT ON COLUMN descriptions in `ingest.py`
- [ ] `python generate_schema.py` runs successfully
- [ ] `schema.yaml` contains all tables with descriptions and row counts

### Phase 6: Data Package
- [ ] `datapackage.json` has a descriptive `name` (lowercase, hyphens)
- [ ] `datapackage.json` has a human-readable `title`
- [ ] `datapackage.json` references `data.duckdb` in `resources`

### Final Verification
- [ ] Can answer "How much X did I do?" type questions
- [ ] Can answer "What are my top Y?" type questions
- [ ] Can answer time-based questions (trends, by month, etc.)
