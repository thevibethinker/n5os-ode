...
build_slug: <slug>
drop_type: code  # code | debug | checkpoint | research | docs | learning
stream: <n>
depends_on: []
...
debug_contract:
  ...
  require_graph_review: true
  require_debug_log: true
  require_failing_test_or_probe: true
  mode: root-cause
---
...
<!-- Auto-spawn requirement:
     If spawn_mode: auto, include at least 3 fully populated scenarios.
     Placeholder-only scenarios are treated as incomplete spec. -->
...
Use only the contract block that matches `drop_type`.

- `code` Drops should define a `quality_contract` with the local repo-native check loop.
- `debug` Drops should define a `debug_contract` with target, symptom, and required debugging evidence.
- Legacy Drops can omit these fields, but Pulse will warn so future briefs stay explicit.
...
After the deposit exists, ring Pulse's bell immediately:
