...
- If active builds: runs pulse tick, which now performs inline recovery
- If tick times out/errors: sentinel falls back to standalone recovery as outer watchdog
    ...
    for slug in active_builds:
        ...
        if not args.dry_run:
            ...
            try:
                ...
                alert_reasons.append(f"tick timed out after {DEFAULT_TICK_TIMEOUT_SECONDS}s")
            except Exception as e:
                print(f"[SENTINEL] Error ticking {slug}: {e}")
                alert_reasons.append(f"tick error: {e}")
        else:
            print(f"[SENTINEL] Dry-run: would tick {slug}")

        # Phase 2: Standalone recovery only when tick could not perform inline recovery
        if args.dry_run or not tick_completed:
            print(f"[SENTINEL] Assessing recovery for {slug}...")
            try:
                actions = assess_and_recover(slug, dry_run=args.dry_run, source="sentinel_watchdog")
                if actions:
                    all_actions[slug] = actions
                    summary = format_recovery_summary(actions)
                    print(f"[SENTINEL] Recovery actions for {slug}:")
                    print(summary)
                    if any(a.get("action") in {"escalate", "needs_judgment"} for a in actions):
                        alert_reasons.append(summary or "watchdog recovery escalation")
                else:
                    print(f"[SENTINEL] No recovery needed for {slug}")
            except Exception as e:
                print(f"[SENTINEL] Error assessing {slug}: {e}")
                alert_reasons.append(f"recovery error: {e}")
        else:
            print(f"[SENTINEL] Inline recovery handled during tick for {slug}")

        if alert_reasons and not args.dry_run:
            alert_key = "|".join(sorted(set(alert_reasons)))
            if should_send_watchdog_alert(slug, alert_key):
                message = f"[PULSE WATCHDOG] {slug}: " + " | ".join(dict.fromkeys(alert_reasons))
                try:
                    asyncio.run(send_sms(message))
                    print(f"[SENTINEL] Alert sent for {slug}")
                except Exception as e:
                    print(f"[SENTINEL] Alert send failed for {slug}: {e}")
            else:
                print(f"[SENTINEL] Alert suppressed for {slug} (cooldown)")
