...
def inject_pool_claim_instructions(brief_content: str, slug: str, drop_id: str = "UNKNOWN") -> str:
    ...
    pulse_scripts_path = str(WORKSPACE / "Skills" / "pulse" / "scripts")
    pool_instructions = f"""
...
```bash
python3 -c "
import sys; sys.path.insert(0, '{pulse_scripts_path}')
...
def _ensure_conversations_table(cursor: sqlite3.Cursor) -> None:
    """Create/upgrade conversations table if needed."""
    cursor.execute(
        """
    ...
    if "completed_at" not in columns:
        cursor.execute("ALTER TABLE conversations ADD COLUMN completed_at TEXT")
...
def register_drop_conversation(drop_id: str, slug: str, convo_id: str):
    ...
    try:
        ...
        cursor = conn.cursor()
        _ensure_conversations_table(cursor)
        now = datetime.now(timezone.utc).isoformat()
        cursor.execute(
            """
        ...
        print(f"[WARN] register_drop_conversation skipped: {e}")
...
def update_drop_conversation_status(convo_id: str, status: str):
    ...
    try:
        ...
        print(f"[WARN] update_drop_conversation_status skipped: {e}")
...
{brief_with_broadcasts}"""
    ...
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=DEFAULT_SPAWN_TIMEOUT)) as session:
        async def do_request(body: dict):
            async with session.post(
                ZO_API_URL,
                headers={
                    "authorization": token,
                ...
                try:
                    ...
                    raise RuntimeError(f"API returned non-JSON body: {text[:500]}")

        result = await do_request(request_body)
        ...
        tracking_id = result.get("conversation_id") or f"pulse_{slug}_{drop_id}_{int(start.timestamp())}"
...
def _can_advance_wave(meta: dict) -> bool:
    ...
    wave_val = waves[active_wave]
    if isinstance(wave_val, dict):
        wave_val = wave_val.get("drops", [])
    for drop_id in wave_val:
...
def get_ready_drops(meta: dict) -> list[str]:
    ...
    if meta.get("waves"):
        ...
        if isinstance(meta.get("gate"), dict) and meta["gate"].get("type") == "wave_blocked":
            return []
        wave_val = (meta.get("waves") or {}).get(active_wave, [])
        if isinstance(wave_val, dict):
            wave_val = wave_val.get("drops", [])
        allowed = set(wave_val or [])
...
def main():
    ...
    if args.command == "start":
        ...
        resumed = resume_build(args.slug)
        if resumed:
            asyncio.run(tick(args.slug))
