...
Examples:
  ...
  %(prog)s --slug my-build              # Force build-close mode
    ...
    if args.slug:
        ...
        cmd = f"python3 /home/workspace/Skills/build-close/scripts/close.py --slug {args.slug}"
        if args.dry_run:
            cmd += " --dry-run"
        if args.force:
            cmd += " --force"
    ...
    if context == "drop":
        ...
        cmd = f"python3 /home/workspace/Skills/drop-close/scripts/close.py --convo-id {convo_id}"
        if args.force:
            cmd += " --force"
        ...
        cmd = f"python3 /home/workspace/Skills/build-close/scripts/close.py --slug {slug}"
        if args.dry_run:
            cmd += " --dry-run"
        if args.force:
            cmd += " --force"
        ...
        cmd = f"python3 /home/workspace/Skills/thread-close/scripts/close.py --convo-id {convo_id}"
        if args.tier:
