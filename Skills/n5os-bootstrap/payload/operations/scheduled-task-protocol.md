...
**2. Agent Conflict Gate**
- Run `python3 {{WORKSPACE}}/N5/scripts/agent_conflict_gate.py --summary --check-log --no-cache`
- Block on exit code 1 unless explicit approval
- Rerun with `--force` if approved
...
## Agent Conflict Gate Protocol

- **Gate script** – Before creating or reactivating any scheduled agent, run `python3 N5/scripts/agent_conflict_gate.py --summary --check-log --no-cache`. The script inventories `list_agents`, applies duplication and conflict heuristics, and emits structured JSON (conflict_type, agent_ids, reason, confidence).
- **Record results** – Append each output to `file 'N5/logs/agent_conflict_gate.log'` (ISO timestamp, exit code, conflict count/types). The log is the source of truth for weekly audits and cleanup tracking.
- **Resolve before block** – If conflicts are reported (exit code 1) and the gate is not yet cleared, pause creation/reactivation until a reviewer either resolves the conflict or documents the override (`--force`). Update `file 'N5/notes/agent-conflict-cleanup.md'` with the actions taken.
- **Weekly audit** – The weekly scheduled task audit must confirm that the gate log is up to date and that any duplicates/stale agents flagged in the log have been addressed or annotated with next steps.
...
```markdown
❌ BAD:
Execute: python3 -c "import os; files = os.listdir('{{WORKSPACE}}/Lists'); print(files)"
...
6. Agent conflict gate log reviewed; conflicts triaged or resolved.
