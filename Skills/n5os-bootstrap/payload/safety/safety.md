...
1. **Check commands registry:**
   ```bash
   grep -i "keyword" {{WORKSPACE}}/N5/data/executables.db
   ```
...
- Do not create files in `{{WORKSPACE}}` root without permission
...
### Agent Conflict Gate (Scheduled Agents)

Before creating or reactivating scheduled agents, run `python3 {{WORKSPACE}}/N5/scripts/agent_conflict_gate.py --summary --check-log --no-cache`. If exit code 1 (conflicts detected), block creation unless V explicitly approves, then rerun with `--force`. If exit code 2 (gate error), warn V and allow creation with confirmation. Log every run to `file 'N5/logs/agent_conflict_gate.log'` with timestamp, exit code, conflict counts/types, and any override note.
