...
# Careerspan Data Quality Methodology

## Overview

This document captures the discrete steps required to transform raw Careerspan Intelligence Brief data into employer-ready, verified output. Zero hallucinations and zero areas of doubt are non-negotiable.
...
- **Use when:** Careerspan data comes as documents (next 2 days)

### Mode B: Structured API Data (Target State)
- **Input:** JSON/YAML from Careerspan API endpoint
- **Requires:** Skip directly to Phase 4 (Structuring)
- **Status:** Pending API availability
- **Use when:** Receiving data programmatically from Careerspan platform
...
- Careerspan Intelligence Brief (Google Doc format)
...
1. `careerspan_full_ocr.txt` — Raw OCR output
2. `careerspan_cleaned.md` — Cleaned, structured document
...
*Careerspan Proprietary Methodology*
