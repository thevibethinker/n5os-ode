#!/usr/bin/env python3
"""
Validate Careerspan decomposer outputs against canonical schema.

Usage:
    python3 validate.py <inbox_path>
    python3 validate.py Careerspan/meta-resumes/inbox/hardik-flowfuse
...
def validate_candidate(candidate_dir: Path, schema: dict) -> dict:
    ...
    if overview_path.exists():
        ...
        try:
            ...
            if score is None:
                results["warnings"].append("careerspan_score.overall is null/missing")
            else:
                results["stats"]["careerspan_score"] = score
...
def main():
    ...
    for candidate_dir in sorted(candidates):
        ...
        if results["stats"]:
            score = results["stats"].get("careerspan_score", "?")
