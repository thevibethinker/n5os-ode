---
name: careerspan-decomposer
description: |
  Decomposes Careerspan Intelligence Briefs into structured YAML using LLM semantic extraction.
  Takes raw Careerspan doc + JD → Outputs indexed YAML files to inbox for Meta-Resume generation.
...
python3 Skills/careerspan-decomposer/scripts/decompose.py \
  --doc /path/to/careerspan_ocr.txt \
  --jd /path/to/jd.yaml \
  --candidate "firstname" \
  --company "company"
...
# Validate outputs
python3 Skills/careerspan-decomposer/scripts/validate.py --all
python3 Skills/careerspan-decomposer/scripts/validate.py hardik-flowfuse
```
...
├── overview.yaml           # Profile summary, careerspan_score
...
**Top-Level Fields:**
- `overall_score`: Integer 0-100, the Careerspan score
...
The careerspan_score is extracted from two formats:

1. **Web format** (preferred): `90\nOverall score`
2. **Legacy format**: `Referred 89 ©`

This is stored in:
- `overview.yaml` → `careerspan_score.overall`
- `manifest.yaml` → `careerspan_score`
...
- **Canonical output schema**: `Skills/careerspan-decomposer/assets/canonical_schema.json`
- **Structured input schema**: `Skills/careerspan-decomposer/assets/input_schema.json`

Use these for validation in downstream systems.

---

*Careerspan Proprietary Skill*
