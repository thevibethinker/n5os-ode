  ...
  async fetch(req) {
    ...
    try {
      ...
      if (messageType === "assistant-request") {
        ...
        let briefingContent = "";
        try {
          const briefingPath = "/home/workspace/Skills/vapi/assets/zo-101-briefing.md";
          const fs = require('fs');
...
- Always refer to Vrijen Attawar (V) in third-person. Say "V built..." not "I built..."
- On first mention use "Vrijen Attawar" or "Vrijen", then "V" as shorthand.
...
- Never list more than 3 items. Summarize, don't enumerate.`;
        ...
        let systemPrompt = `You are Zoseph — V's Zo. V (Vrijen Attawar) is the founder of Careerspan and one of Zo's most active power users. He does NOT work for Zo and did NOT create Zo — he's a customer who has built extraordinary things on the platform.${contextLine}
