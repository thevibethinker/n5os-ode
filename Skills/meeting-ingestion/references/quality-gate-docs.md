...
The `quality_gate.py` script validates meeting readiness before canonical shape generation using a comprehensive 4-stage quality framework. It sits between the 'identified' and 'gated' statuses in the processing pipeline, ensuring only high-quality meetings proceed to processing.
...
3. **Shape Generation**: Canonical meeting artifact validation (not implemented yet - future)
...
Failed checks automatically create HITL queue items in `N5/review/meetings/hitl-queue.jsonl`:
...
Optional YAML config at `N5/config/quality-harness.yaml`:
...
gate = QualityGate(config_path=Path("N5/config/quality-harness.yaml"))
