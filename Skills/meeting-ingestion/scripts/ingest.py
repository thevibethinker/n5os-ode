...
Rules:
- Vrijen Attawar (the user) is always "V" - use "V" not "Vrijen"
...
Examples:
- "2024-01-15_David-x-Careerspan" → ["David", "V"]
- "Maximilian_Kwok_and_Vrijen_Attawar" → ["Maximilian Kwok", "V"]
- "John_Smith_x_V_transcript" → ["John Smith", "V"]
- "2026-01-26_Collateral-Blitz_Logan" → ["Logan", "V"]
- "Alex_x_Vrijen_Wisdom_Partners" → ["Alex", "V"]
...
Respond with ONLY a JSON array like: ["Name1", "Name2"]"""
    ...
    def _fallback_participant_extraction(self, filename: str) -> List[str]:
        ...
        participants = []
        for part in parts:
            if part.lower() in ['vrijen', 'vrijenattawar']:
