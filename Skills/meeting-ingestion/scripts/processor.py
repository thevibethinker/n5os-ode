...
def process_meeting(
    ...
    for block_code in manifest:
        logger.info(f"Generating {block_code}...")
        
        try:
            block_content = generate_block(transcript_text, block_code, context)
            
            # Write block to file
            block_file = meeting_dir / f"{block_code}.md"
            block_file.write_text(block_content)
            logger.info(f"  Written: {block_file}")
            
            results["blocks_generated"].append(block_code)
