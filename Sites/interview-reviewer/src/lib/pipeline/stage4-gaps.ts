  ...
  "requirementCoverage": [
    {
      ...
      "status": "demonstrated" | "partially-covered" | "not-addressed",
...
Return your analysis as JSON.`;
  ...
  const partiallyCovered = normalizedCoverage.filter(r => r.status === "partially-covered").length;
  const notAddressed = normalizedCoverage.filter(r => r.status === "not-addressed").length;
...
function normalizeCoverageStatus(status: string): CoverageStatus {
  ...
  if (normalized.includes("partial")) return "partially-covered";
