...
def cmd_close(slug: str, force: bool = False):
    """Mark build as complete."""
    build_dir = get_build_dir(slug)
    
    meta_file = build_dir / "meta.json"
    
    # Handle builds without meta.json
    if not meta_file.exists():
        meta = {
            "slug": slug,
            ...
            "status": "active",
            "type": "unknown",
            "created": get_iso_timestamp()[:10],
            "workers": {"total": 0, "complete": 0}
        }
        print(f"ℹ️  No meta.json found - creating minimal metadata")
    else:
    ...
    if complete < total and not force:
...
def main():
    ...
    close_parser.add_argument(
        "--force", "-f",
        action="store_true",
        help="Skip confirmation for incomplete builds"
    )
    ...
    if args.command == "complete":
        ...
        cmd_close(args.slug, args.force)
