...
def get_daily_context():
    """Retrieve daily context (calendar, etc) for reflection prompts."""
    context_path = Path("/home/workspace/N5/data/daily_context.md")
    if context_path.exists():
        try:
            return "\n\n## Daily Context\n" + context_path.read_text().strip() + "\n"
        except Exception:
            return ""
    return ""
...
def init_db():
    ...
    # Check if diet column exists (for migration)
    cursor.execute("PRAGMA table_info(journal_entries)")
    columns = [info[1] for info in cursor.fetchall()]
    if 'diet' not in columns:
        cursor.execute("ALTER TABLE journal_entries ADD COLUMN diet TEXT")
...
def new_entry(entry_type: str, mood: str = None, diet: str = None, tags: str = None):
    ...
    # Inject daily context for evening reflections
    if entry_type == "evening":
        prompt += get_daily_context()
    ...
    cursor.execute("""
        INSERT INTO journal_entries (entry_type, content, mood, diet, tags, word_count)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (entry_type, content, mood, diet, tags, word_count))
...
def list_tags():
    """List all tags with usage counts and show structured tag categories."""
    ...
    cursor.execute("SELECT tags FROM journal_entries WHERE tags IS NOT NULL AND tags != ''")
    rows = cursor.fetchall()
    conn.close()
...
def by_tag(tag: str, limit: int = 20):
    """List entries with a specific tag or tag category."""
    conn = get_db()
    cursor = conn.cursor()
    
    # Support category queries like "mood:" to find all mood:* tags
    if tag.endswith(':'):
        pattern = f"%{tag}%"
    else:
        pattern = f"%{tag}%"
    ...
    cursor.execute("""
        SELECT id, created_at, entry_type, tags, word_count, substr(content, 1, 60) as preview
        FROM journal_entries
        WHERE tags LIKE ?
        ORDER BY created_at DESC
        LIMIT ?
    ...
    if not rows:
        print(f"No entries with tag matching '{tag}'")
        return
    
    print(f"\nEntries tagged with '{tag}':")
    print(f"\n{'ID':>5} | {'Date':^19} | {'Type':^15} | Tags")
    print("-" * 90)
    
    for row in rows:
        created = row['created_at'][:16] if row['created_at'] else 'Unknown'
        tags = row['tags'][:30] + '...' if len(row['tags'] or '') > 30 else (row['tags'] or '')
        print(f"{row['id']:>5} | {created:^19} | {row['entry_type']:^15} | {tags}")
    
    print(f"\n{len(rows)} entries found")


def add_entry(entry_type: str, content: str, mood: str = None, diet: str = None, tags: str = None, date: str = None):
    ...
    created_at = date if date else datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    cursor.execute("""
        INSERT INTO journal_entries (entry_type, content, mood, diet, tags, word_count, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (entry_type, content, mood, diet, tags, word_count, created_at))
...
def main():
    ...
    new_parser.add_argument('--diet', help='Diet summary')
    ...
    add_parser.add_argument('--diet', help='Diet summary')
    add_parser.add_argument('--tags', help='Comma-separated tags')
    add_parser.add_argument('--date', help='Custom date (YYYY-MM-DD HH:MM:SS)')
    
    # tags command
    tags_parser = subparsers.add_parser('tags', help='List tags and filter by category')
    tags_parser.add_argument('tag', nargs='?', help='Tag or category (e.g., mood:, work)')
    tags_parser.add_argument('--limit', '-n', type=int, default=20, help='Max entries to show')
    
    args = parser.parse_args()
    
    if args.command == 'new':
        new_entry(args.type, args.mood, args.diet, args.tags)
        ...
        add_entry(args.type, args.content, args.mood, args.diet, args.tags, args.date)
    elif args.command == 'tags':
        if args.tag:
            by_tag(args.tag, args.limit)
        else:
            list_tags()
    else:
        parser.print_help()
