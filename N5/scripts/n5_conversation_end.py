...
def save_proposed_title():
    ...
    try:
        ...
        # Find the latest thread archive
        ...
        if not archives:
            logger.debug("No thread archives found")
            return
        ...
        if not aar_file.exists():
            logger.debug(f"No AAR file found: {aar_file}")
            return
        ...
        if CONVERSATION_WS:
            ...
            if CONVERSATION_REGISTRY_AVAILABLE:
                ...
                if registry.update(convo_id, title=titles[0]['title']):
                    logger.info(f"✓ Updated conversation title in database: {convo_id}")
                else:
                    logger.debug(f"Conversation {convo_id} not in database (skipping title update)")
