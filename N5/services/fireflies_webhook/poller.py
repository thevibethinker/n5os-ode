...
class WebhookPoller:
    ...
    def start(self):
        """Start polling loop"""
        logger.info(f"Starting webhook poller (interval={self.interval}s, batch={self.batch_size})")
        ...
        if not self.processor.fireflies_client.test_connection():
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fireflies Webhook Poller")
