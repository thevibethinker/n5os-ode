...
def scan_for_new_transcripts():
    """Scan watch directories for unprocessed transcripts - IDEMPOTENT"""
    logger.info("Scanning for new transcripts...")
    
    # Database check (idempotency layer 1)
    conn = sqlite3.connect(PIPELINE_DB)
    cursor = conn.cursor()
    cursor.execute("SELECT meeting_id FROM meetings")
    processed_in_db = {row[0] for row in cursor.fetchall()}
    conn.close()
    ...
    for watch_dir in WATCH_DIRS:
        ...
        for tf in watch_dir.glob("**/*.transcript.md"):
            ...
            # Database check (idempotency layer 1)
            if meeting_id not in processed_in_db:
...
def main(dry_run=False):
    ...
    Phase 5 (Python): Save + finalize + mark processed
    """
    logger.info("Meeting Pipeline - Transcript Processor v2")
    if dry_run:
        logger.info("DRY RUN MODE")
    
    new_transcripts = scan_for_new_transcripts()
    if len(new_transcripts) == 0:
        logger.info("No new transcripts (idempotency working)")
        return 0
