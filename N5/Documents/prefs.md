...
> ⚠️ **CRITICAL: DO NOT OVERWRITE THIS ENTIRE [prefs.md](http://prefs.md) FILE.**
>
> - Always review the FULL file before making any changes.
> - Append incremental edits ONLY, with explicit version bumps.
> - Log all modifications in the Change Log section.
> - Failure to follow this protocol risks LOSS OF DATA and COMPROMISES SYSTEM INTEGRITY.
>
> Adhere to these rules STRICTLY to protect core system stability and traceability.

This governs defaults and rules. Workflow sub-preferences may override; project \_prefs.md overrides those.

## Command Index (top)

- `jobs-add` — Add one-off job to private list (see ./commands/jobs-add.md)
- `jobs-review` — TUI to approve/reject PENDING jobs (see ./commands/jobs-review.md)
- `jobs-scrape` — Scrape open roles from given company list (TXT file) (see ./commands/jobs-scrape.md)
- `transcript-ingest` — Ingest and process transcripts from files or Google Drive folders (see ./commands/transcript-ingest.md)
...
**Highest Priority Governance**: Folder-specific [POLICY.md](http://POLICY.md) files take precedence over these global preferences unless explicitly exempted in the policy file itself (e.g., "Exempts: Safety Overrides"). Policies govern the collective interpretation and handling of folder contents as programs, databases, or dynamic entities.

- **Mandatory Check**: Always scan for and consult [POLICY.md](http://POLICY.md) in the target folder before any interaction (read, edit, add, delete). If absent, default to this global [prefs.md](http://prefs.md) but flag for policy creation.

- **Anchors System**: Each [POLICY.md](http://POLICY.md) must include an "Anchors" section linking to root N5/prefs.md, related issues, or parent policies for cross-referencing and system coherence.

- **Overrides Mechanism**: Folder policies can override any global rule; document exemptions clearly. Conflicts resolved by escalating to root [POLICY.md](http://POLICY.md) or user arbitration.

- **Naming Convention**: Use "[POLICY.md](http://POLICY.md)" for consistency and easy sourcing by title (e.g., search for "[POLICY.md](http://POLICY.md)" in folder tree).

- **Creation Protocol**: When creating a new folder, always generate [POLICY.md](http://POLICY.md) first. Include purpose, handling rules, safety flags, dependencies, and anchors.
...
- **File Classification System**: Files are protected differently based on their role:
  ...
  **HARD PROTECTION** (Manual-Edit Only)
  ...
  - [N5.md](http://N5.md) - Core system index (hand-authored)
...
Project \_prefs.md &gt; Workflow sub-pref &gt; Global [prefs.md](http://prefs.md). Knowledge informs, does not override.
