...
- ☐ Create contradiction detection in edge_extractor (flag conflicts with existing edges)
- ☐ Test: Full pipeline on new meeting captures edges automatically

### Phase 4: Analysis Layer (The "So What")
- ☐ Create `N5/scripts/analysis/decision_retrospective.py` — Outcome validation
- ☐ Create `N5/scripts/analysis/reversal_detector.py` — Find flip-flop patterns
- ☐ Create `N5/scripts/analysis/influence_map.py` — Who shapes V's thinking
- ☐ Create `N5/scripts/analysis/originated_vs_adopted.py` — Pattern analysis
- ☐ Create `N5/scripts/analysis/decay_detector.py` — Stale edges
- ☐ Test: Run each analysis on seeded data, output makes sense
...
### Affected Files
- `N5/scripts/analysis/decision_retrospective.py` - CREATE
- `N5/scripts/analysis/reversal_detector.py` - CREATE
- `N5/scripts/analysis/influence_map.py` - CREATE
- `N5/scripts/analysis/originated_vs_adopted.py` - CREATE
- `N5/scripts/analysis/decay_detector.py` - CREATE
...
- Finds all `hoped_for` and `concerned_about` edges older than 90 days
- Groups by decision/idea
- Checks if outcome was linked
- Outputs report: "These decisions need outcome review"

```bash
python3 decision_retrospective.py --since 2025-10-01 --output report.md
```

**4.2 Reversal Detector (`reversal_detector.py`):**

Quarterly analysis that:
- Finds all `reversed` and `superseded` edges
- Groups by topic/domain
- Identifies patterns: "You've changed direction 4x on pricing strategy"

**4.3 Influence Map (`influence_map.py`):**

Quarterly analysis that:
- Counts `originated_by` and `influenced_by` edges per person
- Breaks down by topic/domain
- Outputs: "These 5 people have shaped 40% of your product thinking"

**4.4 Originated vs Adopted (`originated_vs_adopted.py`):**

Quarterly analysis that:
- Separates ideas by `originated_by: vrijen` vs others
- Tracks outcome rates (validated vs invalidated)
- Outputs: "Your originated ideas stick 40% of the time; adopted ideas stick 70%"

**4.5 Decay Detector (`decay_detector.py`):**

Weekly analysis that:
- Finds `active` edges not reviewed in 90+ days
- Finds commitments without follow-up edges
- Outputs: "These 12 decisions may be stale"

### Unit Tests

- **Test 1:** Seed 20 edges with varied statuses → Retrospective identifies correct subset
- **Test 2:** Seed 5 reversals on same topic → Detector groups them
- **Test 3:** Seed edges from 3 people → Influence map ranks correctly
