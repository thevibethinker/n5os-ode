...
Build folder: scripts/builds/<slug>/
```
...
- Commands: `python3 scripts/pulse/queue_manager.py add|list|prioritize|advance|next`

### Interview System
- Fragment-based async collection
- Multi-channel aggregation (SMS + email + chat)
- JSONL storage: `scripts/pulse/interviews/<task-id>/fragments.jsonl`
- Seeded judgment with LLM evaluation
- Commands: `python3 scripts/pulse/interview_manager.py start|add|status|seed`
...
- Commands: `python3 scripts/pulse/plan_sync.py sync|notify|approve`
...
- Commands: `python3 scripts/pulse/telemetry_manager.py log|query|export`
- Requirements: `python3 scripts/pulse/requirements_tracker.py capture|list|export`
...
```bash
python3 scripts/pulse/review_manager.py initiate my-build --force
```
...
Two tiers:
1. **Build-local** → `scripts/builds/<slug>/BUILD_LESSONS.json`
2. **System-wide** → `scripts/learnings/SYSTEM_LEARNINGS.json`
...
- `file 'scripts/learnings/SYSTEM_LEARNINGS.json'` — System-wide learnings
- `file 'scripts/config/pulse_control.json'` — Sentinel control state
- `file 'Documents/System/Build-Orchestrator-System.md'` — Legacy manual system
- `file 'scripts/pulse/'` — v2 scripts directory
