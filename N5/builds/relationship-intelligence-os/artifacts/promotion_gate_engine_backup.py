...
class PromotionGateScorer:
    ...
    def _score_strategic_importance(self, input_data: ScoringInput) -> float:
        ...
        if strategic_matches >= 3:
            score += 15
            explanation.append(f"High strategic content ({strategic_matches} indicators)")
        elif strategic_matches >= 1:
            score += 8
            explanation.append(f"Medium strategic content ({strategic_matches} indicators)")
        else:
            score += 2
            explanation.append("Limited strategic content")
        ...
        if input_data.meeting_context:
            ...
            if has_executives:
                score += 3
                explanation.append("Executive participation")
    ...
    def _score_commitment_clarity(self, input_data: ScoringInput) -> float:
        """Score commitment clarity - owner/date/scope (0-20)."""
        score = 0.0
        
        candidate = input_data.candidate_data
        
        # Look for commitment elements
        has_owner = False
        has_date = False  
        has_scope = False
        
        # Check for explicit commitment structure
        if 'commitment_details' in candidate:
            commitment = candidate['commitment_details']
            
            if 'owner' in commitment and commitment['owner']:
                has_owner = True
                score += 7
            ...
            if any(k in commitment for k in ['scope', 'description', 'deliverable_type']):
                has_scope = True
                score += 6
        
        # Check for implicit commitments in content
        content_text = self._extract_text_content(candidate)
        
        # Owner indicators
        if not has_owner:
            owner_indicators = ['i will', 'we will', 'team will', 'responsible for']
            if any(indicator in content_text.lower() for indicator in owner_indicators):
                has_owner = True
                score += 4
        
        # Date indicators  
        if not has_date:
            date_indicators = ['deadline', 'due', 'by next', 'end of', 'this week', 'this month']
            if any(indicator in content_text.lower() for indicator in date_indicators):
                has_date = True
                score += 4
        
        # Scope indicators
        if not has_scope:
            scope_indicators = ['deliverable', 'outcome', 'result', 'proposal', 'document']
            if any(indicator in content_text.lower() for indicator in scope_indicators):
                has_scope = True
                score += 3
        
        # Clarity bonus for having all three elements
        if has_owner and has_date and has_scope:
            score += 3
    ...
    def _score_evidence_quality(self, input_data: ScoringInput) -> float:
        ...
        for q in quotes:
            if isinstance(q, str):
                ...
                quote_texts.append(q.get('text', ''))
            else:
                quote_texts.append('')
        
        if quote_texts:
            avg_quote_length = sum(len(text) for text in quote_texts) / len(quote_texts)
    ...
    def _score_novelty(self, input_data: ScoringInput) -> float:
        ...
        if existing_memory:
            ...
            for memory_item in existing_memory:
                ...
                similarity = self._calculate_text_similarity(candidate_content, memory_content)
                similarities.append(similarity)
            
            max_similarity = max(similarities) if similarities else 0
            
            # Score based on novelty (inverse of similarity)
            if max_similarity >= 0.8:
                score = 2  # Very similar to existing
            elif max_similarity >= 0.6:
                score = 5  # Somewhat similar
            elif max_similarity >= 0.4:
                score = 8  # Somewhat novel
    ...
    def _explain_commitment_score(self, score: float) -> str:
        """Explain commitment clarity score."""
        if score >= 15:
            return "Clear commitment with owner, timeline, and scope"
        elif score >= 10:
            return "Partial commitment clarity"
        elif score >= 5:
            return "Some commitment elements present"
        else:
    ...
    def _explain_evidence_score(self, score: float) -> str:
        ...
        if score >= 12:
            return "Strong evidence with multiple detailed quotes"
        elif score >= 8:
            return "Good evidence with specific quotes"
        elif score >= 4:
    ...
    def _explain_novelty_score(self, score: float) -> str:
        """Explain novelty score."""
        if score >= 12:
            return "Novel information not in existing memory"
        elif score >= 8:
            return "Somewhat new information"
        elif score >= 5:
            return "Some overlap with existing knowledge"
        else:
            return "Largely duplicate information"
    
    def _explain_execution_score(self, score: float, candidate_type: str) -> str:
        """Explain execution value score."""
        if score >= 8:
            return "High actionability with clear next steps"
        elif score >= 5:
            return "Moderate actionability"
        elif score >= 3:
            return "Some actionable elements"
        else:
            return "Primarily informational value"
    ...
    def _calculate_text_similarity(self, text1: str, text2: str) -> float:
        """Simple text similarity calculation (placeholder for real similarity)."""
        
        # Convert to word sets for simple Jaccard similarity
...
class PromotionGateRouter:
    """Routing engine for promotion decisions."""
    
    def __init__(self, config: ScoringConfig):
        self.config = config
    
    def determine_routing(self, scoring_result: ScoringResult, candidate_type: str) -> RoutingDecision:
        """Determine routing for a scored candidate."""
        
        tier = scoring_result.tier
        
        # Get base routing from config
        base_routing = self.config.routing_rules[tier].copy()
        
        # Adjust routing based on candidate type
        routing = self._adjust_routing_for_type(base_routing, candidate_type)
        
        # Determine status
        if scoring_result.hard_override and scoring_result.hard_override['applied']:
            status = 'promoted'
            reasoning = f"Hard override: {scoring_result.hard_override['reason']}"
        elif tier == 'A':
            status = 'promoted'
            reasoning = f"Tier A: Auto-promote (score: {scoring_result.total_score})"
        elif tier == 'B':
            status = 'queued_for_review'
            reasoning = f"Tier B: Review digest (score: {scoring_result.total_score})"
        else:
            status = 'archived'
            reasoning = f"Tier C: Archive (score: {scoring_result.total_score})"
    ...
    def _adjust_routing_for_type(self, base_routing: Dict[str, bool], candidate_type: str) -> Dict[str, bool]:
        """Adjust routing based on candidate type."""
...
class PromotionGateEngine:
    ...
    def _create_promotion_event(self, 
        ...
        # Add score explanations as metadata
